/**
 * KIMI K2 Service - Moonshot KIMI K2 Integration via Anthropic-compatible API
 *
 * Interacts with KIMI K2 models through Anthropic-compatible API endpoints.
 * Uses Claude Agent SDK with custom base URL and auth token.
 */
...
import { KIMI_DEFAULT_MODEL, normalizeKimiModelId, getKimiModelDisplayName } from '@/lib/constants/kimiModels';
...
const dispatchToolMessage = async ({
  ...
  try {
    ...
    console.error('[KimiService] Failed to persist tool message:', error);
  }
};
...
/**
 * Execute command using KIMI K2 via Anthropic-compatible API
 *
 * @param projectId - Project ID
 * @param projectPath - Project directory path
 * @param instruction - Command to pass to AI
 * @param model - KIMI model to use (default: kimi-k2-thinking-turbo)
...
export async function executeKimi(
  ...
  console.log(`\n========================================`);
  console.log(`[KimiService] 🚀 Starting KIMI K2 Service`);
  console.log(`[KimiService] Project: ${projectId}`);
  const resolvedModel = resolveModelId(model);
  const modelLabel = getKimiModelDisplayName(resolvedModel);
  const aliasNote = resolvedModel !== model ? ` (alias for ${model})` : '';
  console.log(`[KimiService] Model: ${modelLabel} [${resolvedModel}]${aliasNote}`);
  console.log(`[KimiService] Session ID: ${sessionId || 'new session'}`);
  console.log(`[KimiService] Instruction: ${instruction.substring(0, 100)}...`);
  console.log(`========================================\n`);
  ...
  // Configure environment for KIMI K2
  process.env.ANTHROPIC_BASE_URL = 'https://api.moonshot.cn/anthropic';
  process.env.ANTHROPIC_AUTH_TOKEN = 'sk-7ay6ofQGqPPiaWd49K7COLlzg16iJj3xK9tibHYdbgOgagNM';
  process.env.ANTHROPIC_MODEL = resolvedModel;
  process.env.ANTHROPIC_SMALL_FAST_MODEL = resolvedModel;

  console.log(`[KimiService] ✅ Environment configured for KIMI K2`);
  console.log(`[KimiService] Base URL: ${process.env.ANTHROPIC_BASE_URL}`);
  ...
  const safeMarkRunning = async () => {
    ...
    try {
      ...
      console.error(`[KimiService] Failed to mark request ${requestId} as running:`, error);
    }
  };
  ...
  const safeMarkCompleted = async () => {
    ...
    try {
      ...
      console.error(`[KimiService] Failed to mark request ${requestId} as completed:`, error);
    } finally {
  ...
  const safeMarkFailed = async (message?: string) => {
    ...
    try {
      ...
      console.error(`[KimiService] Failed to mark request ${requestId} as failed:`, error);
    } finally {
      hasMarkedTerminalStatus = true;
    }
  };
  ...
  publishStatus('starting', 'Initializing KIMI K2...');
  ...
  try {
    ...
    console.log(`[KimiService] ✅ Project verified: ${project.name}`);

    // Validate and prepare project path
    console.log(`[KimiService] 🔒 Validating project path...`);
    ...
    try {
      ...
      console.log(`[KimiService] ✅ Project directory exists: ${absoluteProjectPath}`);
    } catch {
      console.log(`[KimiService] 📁 Creating project directory: ${absoluteProjectPath}`);
      await fs.mkdir(absoluteProjectPath, { recursive: true });
    }

    // Send ready notification via SSE
    publishStatus('ready', 'Project verified. Starting KIMI K2...');

    // Start Claude Agent SDK query with KIMI endpoint
    console.log(`[KimiService] 🤖 Querying KIMI K2 via Anthropic SDK...`);
    console.log(`[KimiService] 📁 Working Directory: ${absoluteProjectPath}`);
...
- Prefer giving the user the live preview link that is actually running rather than written instructions.`,
        ...
        stderr: (data: string) => {
          ...
          console.error(`[KimiSDK][stderr] ${line}`);
        },
      } as any,
    });
    ...
    for await (const message of response) {
            ...
            if (isPlaceholderLine) {
              ...
              if (shouldHandle) {
                try {
                  ...
                  console.error('[KimiService] Failed to handle streaming tool placeholder:', error);
                }
              }
              streamState.content = '';
              streamState.hasSentUpdate = false;
              ...
              if (isPlaceholderLine) {
                ...
                if (shouldHandle) {
                  try {
                    ...
                    console.error('[KimiService] Failed to handle tool placeholder on stop:', error);
                  }
                }
              }
      ...
      if (message.type === 'system' && message.subtype === 'init') {
        ...
        console.log(`[KimiService] Session initialized: ${currentSessionId}`);

        // Save session ID to project
        if (currentSessionId) {
          await updateProject(projectId, {
            activeKimiSessionId: currentSessionId,
          });
        }
              ...
              if (isPlaceholderLine) {
                ...
                if (shouldHandle) {
                  try {
                    ...
                    console.error('[KimiService] Failed to handle assistant tool placeholder:', error);
                  }
                }
                continue;
              }
        ...
        console.log('[KimiService] Assistant message:', content.substring(0, 100));
        ...
        // Final result
        console.log('[KimiService] Task completed:', message.subtype);
    ...
    console.log('[KimiService] Streaming completed');
    await safeMarkCompleted();
    ...
    console.error(`[KimiService] Failed to execute KIMI:`, error);
    ...
    if (error instanceof Error) {
      ...
      // Detect authentication failure
        ...
        errorMessage = `KIMI API authentication failed. Please check your API key.`;
      }
      // Permission error
      ...
      // Generic process exit without details – attempt to surface last stderr lines
        ...
        if (/auth\s+login|not\s+logged\s+in|sign\s+in/i.test(tail)) {
          errorMessage = `KIMI API authentication required.\n\nPlease check your API key.\n\nDetailed log:\n${tail}`;
        } else if (/network|ENOTFOUND|ECONN|timeout/i.test(tail)) {
          errorMessage = `Failed to run KIMI K2 due to network error. Please check your network connection and try again.\n\nDetailed log:\n${tail}`;
    ...
    if (originalEnv.ANTHROPIC_SMALL_FAST_MODEL !== undefined) {
      process.env.ANTHROPIC_SMALL_FAST_MODEL = originalEnv.ANTHROPIC_SMALL_FAST_MODEL;
    } else {
      delete process.env.ANTHROPIC_SMALL_FAST_MODEL;
    }
    console.log(`[KimiService] ✅ Environment variables restored`);
  }
}

/**
 * Initialize Next.js project with KIMI K2
 *
 * @param projectId - Project ID
 * @param projectPath - Project directory path
...
export async function initializeNextJsProject(
  ...
  console.log(`[KimiService] Initializing Next.js project: ${projectId}`);
...
`.trim();
  ...
  await executeKimi(projectId, projectPath, fullPrompt, model, undefined, requestId);
}
...
export async function applyChanges(
  ...
  console.log(`[KimiService] Applying changes to project: ${projectId}`);
  await executeKimi(projectId, projectPath, instruction, model, sessionId, requestId);
}
